...
// The "how the app should behave" knobs — goal, frequency, units, equipment.
// Split out of Profile (which is now your identity/achievements) because these
// are settings, not who you are.
export function SettingsClient() {
  ...
  const save = async (next: Profile) => {
    ...
    setSaved(false);
    try {
      const savedProfile = await api<Profile>("/api/profile", {
        method: "PUT",
        body: JSON.stringify({
          name: next.name,
          goal: next.goal,
  ...
  const toggleEquip = (e: Equipment) => {
    ...
    save({ ...p, equipment: has ? p.equipment.filter((x) => x !== e) : [...p.equipment, e] });
  };
  ...
  return (
    <div className="flex flex-col gap-5 pb-4">
      ...
      <section>
        <SectionTitle>Equipment you have</SectionTitle>
